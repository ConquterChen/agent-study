...
def _get_workdir_root():
    workdir_root = os.environ.get('WORKDIR_ROOT', "agent_from_0t1/data/llm_result")
    return workdir_root
...
def append_to_file(filename, content):
    name = os.path.join(WORKDIR_ROOT, filename)
    if not os.path.exists(name):
        f"{name} not exit, please check file exist before read"
    with open(name, 'a') as f:
        f.write(content)
    whole_content = read_file(filename)
    return f"append_content to file success.this file's name is {filename} and the whole content is {whole_content}"
...
def write_to_file(filename, content):
    ...
    with open(name, 'w', encoding='utf-8') as f:
        f.write(content)
    return f"append_content to file success.this file's name is {filename} and it's contents is {content}"
...
tools_info = [
    ...
    {
        "name": "append_to_file",
        "description": "if you had created a file before ,append llm content to the end of file",
        "args": [
            {
                "name": "filename",
                "type": "string",
...
{
        "name": "write_to_file",
        "description": "create or overwrite a new file ,write llm content to file for exampele if you wanna optimize this file's content",
        "args": [
            {
                "name": "filename",
                "type": "string",
